...
```typescript
{
  ...
  time_taken_minutes: Number,     // Time taken to complete quiz
...
const canAccessChapter = async (chapterId: string) => {
  try {
    const response = await fetch(`/api/progress/chapters/${chapterId}/can-access`, {
  ...
  try {
    const response = await fetch('/api/progress/quiz/submit', {
      ...
      body: JSON.stringify({
        ...
        answers: answers,
        total_time_taken_minutes: calculateTimeTaken()
      })
    });
