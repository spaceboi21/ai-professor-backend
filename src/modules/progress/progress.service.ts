...
export class ProgressService {
  ...
  answers.forEach((answer, index) => {
    ...
    if (hasStoredCorrectAnswer) {
      ...
      const userAnswers = (answer.selected_answers || []).map((a) =>
        a.trim().toLowerCase(),
      );
      ...
      directlyValidatedResults.push({
        ...
        feedback: isCorrect
          ? 'Correct! Well done.'
          : `Incorrect. Correct answer(s): ${quiz.answer.join(', ')}`,
        score: isCorrect ? 1 : 0,
      });
    } else {
  ...
  attempt.is_passed = scorePercentage >= attempt.passing_threshold;
  attempt.ai_verification_result = {
    ...(aiVerificationResult || {}),
    questions_results: allResults,
  ...
  return {
    ...
    data: {
      ...
      ai_verification: aiVerificationResult ? 'completed' : 'skipped',
      ai_verification_report: attempt.ai_verification_result,
