...
export class SchoolAdminService {
  ...
  async getEnhancedDashboard(user: JWTUserPayload, filterDto: DashboardFilterDto) {
    this.logger.log(`Getting enhanced dashboard for user: ${user.id} with role: ${user.role.name}`);
    ...
    try {
      ...
      if (filterDto.from_date) {
        ...
        if (isNaN(fromDate.getTime())) {
          throw new BadRequestException('Invalid from_date format. Use YYYY-MM-DD');
        }
      ...
      if (filterDto.to_date) {
        ...
        if (isNaN(toDate.getTime())) {
          throw new BadRequestException('Invalid to_date format. Use YYYY-MM-DD');
        }
      ...
      const [totalStudents, activeStudents, moduleProgresses, modules] = await Promise.all([
        StudentModel.countDocuments({ deleted_at: null }),
        StudentModuleProgressModel.distinct('student_id', activeStudentsQuery),
      ...
      const totalModules = user.role.name === RoleEnum.PROFESSOR 
      ...
      try {
        if (moduleProgresses.length > 0) {
          ...
          moduleProgresses.forEach(progress => {
            const moduleId = progress.module_id?._id?.toString() || progress.module_id?.toString();
          ...
          const totalProgress = moduleProgresses.reduce((sum, progress) => 
            sum + (progress.progress_percentage || 0), 0);
          averageCompletionPercentage = Math.round(totalProgress / moduleProgresses.length);
        }
      } catch (error) {
        this.logger.warn('Error calculating overview statistics:', error?.message);
      ...
      try {
        modulePerformance = this.calculateModulePerformance(moduleProgresses, modules);
      } catch (error) {
        this.logger.warn('Error calculating module performance:', error?.message);
      }
      ...
      try {
        engagementMetrics = this.calculateEngagementMetrics(moduleProgresses, modules);
      } catch (error) {
        this.logger.warn('Error calculating engagement metrics:', error?.message);
      }

      // Add debug logging
      this.logger.log(`Dashboard stats - Active students: ${activeStudents.length}, Total students: ${totalStudents}, Total modules: ${totalModules}, Active modules: ${activeModules}`);
      ...
      this.logger.error('Error getting enhanced dashboard', error?.stack || error);
  ...
  private analyzeAIFeedbackErrors(feedbacks: any[]): any[] {
    ...
    feedbacks.forEach(feedback => {
      ...
      const studentId = feedback.student_id?._id?.toString() || feedback.student_id?.toString();
      const moduleId = feedback.module_id?._id?.toString() || feedback.module_id?.toString();
    ...
    return Object.entries(errorTypes)
      .map(([errorType, data]: [string, any]) => ({
        ...
        count: data.count,
        percentage: totalFeedbacks > 0 ? Math.round((data.count / totalFeedbacks) * 100 * 10) / 10 : 0,
  ...
  private calculateModulePerformance(progresses: any[], modules: any[]): any[] {
    ...
    progresses.forEach(progress => {
      ...
      moduleStats[moduleId].total_time_spent += progress.progress_percentage * 2; // 2 minutes per percentage point
    });

    return Object.values(moduleStats).map((stats: any) => ({
      module_id: stats.module_id,
      title: stats.title,
      completion_percentage: stats.total_students > 0 
        ? Math.round((stats.completed_students / stats.total_students) * 100) 
        : 0,
      active_students: stats.total_students,
      average_time_spent: stats.total_students > 0 
  ...
  private calculateEngagementMetrics(progresses: any[], modules: any[]): any {
    ...
    const completedProgresses = progresses.filter(p => p.status === ProgressStatusEnum.COMPLETED);
    const completionRate = totalViews > 0 ? Math.round((completedProgresses.length / totalViews) * 100) : 0;
    
    // Calculate average session duration (simplified)
    const totalTimeSpent = progresses.reduce((sum, progress) => sum + (progress.progress_percentage * 2), 0);
    const averageSessionDuration = totalViews > 0 ? Math.round(totalTimeSpent / totalViews) : 0;
