...
export class CommunityService {
    ...
    try {
      ...
      for (const discussion of discussions) {
        ...
        if (discussion.created_by_user && discussion.created_by_user.email) {
          discussion.created_by_user.email = this.emailEncryptionService.decryptEmail(discussion.created_by_user.email);
        }

        // Decrypt email in last_reply_user if it exists
        if (discussion.last_reply_user && discussion.last_reply_user.email) {
          discussion.last_reply_user.email = this.emailEncryptionService.decryptEmail(discussion.last_reply_user.email);
        }
      }
    ...
    try {
      ...
      for (const reply of replies) {
        ...
        if (reply.created_by_user && reply.created_by_user.email) {
          reply.created_by_user.email = this.emailEncryptionService.decryptEmail(reply.created_by_user.email);
        }

        // Decrypt emails in mentions
        if (reply.mentions && Array.isArray(reply.mentions)) {
          for (const mention of reply.mentions) {
            if (mention.mentioned_user_details && mention.mentioned_user_details.email) {
              mention.mentioned_user_details.email = this.emailEncryptionService.decryptEmail(mention.mentioned_user_details.email);
            }
          }
        }
      }
    ...
    try {
      ...
      for (const subReply of subReplies) {
        ...
        if (subReply.created_by_user && subReply.created_by_user.email) {
          subReply.created_by_user.email = this.emailEncryptionService.decryptEmail(subReply.created_by_user.email);
        }
  ...
  async getPinnedDiscussions(
    ...
    try {
      ...
      for (const discussion of discussions) {
        ...
        if (discussion.created_by_details && discussion.created_by_details.email) {
          discussion.created_by_details.email = this.emailEncryptionService.decryptEmail(discussion.created_by_details.email);
        }
      }
    ...
    try {
      ...
      for (const mention of mentions) {
        ...
        if (mention.mentioned_by_user && mention.mentioned_by_user.email) {
          mention.mentioned_by_user.email = this.emailEncryptionService.decryptEmail(mention.mentioned_by_user.email);
        }
      }
  ...
  private async getFilteredStudents(
    ...
    return students.map((student: any) => {
      ...
      const decryptedEmail = this.emailEncryptionService.decryptEmail(student.email || '');
  ...
  private async getFilteredProfessors(
    ...
    return professors.map((professor: any) => {
      // Decrypt the email before returning
      const decryptedEmail = this.emailEncryptionService.decryptEmail(professor.email || '');
    ...
    try {
      ...
      for (const like of likes) {
        ...
        if (like.liked_by_user && like.liked_by_user.email) {
          like.liked_by_user.email = this.emailEncryptionService.decryptEmail(like.liked_by_user.email);
        }
      }
